...
int l_c3_drawBitmap(lua_State *L) {
    ...
    } else if (strcmp(name, "image_RFIDBigChip_bits") == 0) {
        bitmapPointer = image_RFIDBigChip_bits;
    } else if (strcmp(name, "image_C3_bits") == 0) {
        bitmapPointer = image_C3_bits;
    }
...
int l_c3_get_weather(lua_State *L) {
    ...
    String url = "http://api.openweathermap.org/data/2.5/weather?q=" + String(city) +
                 ",ID&units=metric&appid=" + String(apiKey);
