...
void WS_drawLiveScan() {
    ...
    // List SSID yang ditemukan secara real-time
    display.setFont(u8g2_font_6x10_tf);
    int count = WiFi.scanComplete();
    if (count > 0) {
        for (int i = 0; i < count && i < 4; i++) {  // Tampilkan 4 teratas dulu
...
void startScan() {
    ...
    while (true) {
        ...
        // Animasi progress bar palsu agar smooth
    ...
    // --- MODE SELEKSI (LIST LENGKAP) ---
    WS_selected = 0;
    WS_smoothScroll = 0;
    WS_running = true;

    detachCallback();
    btnUp.attachClick([]() { if (WS_selected > 0) WS_selected--; });
    btnDown.attachClick([]() { if (WS_selected < WS_networkCount - 1) WS_selected++; });
    btnOK.attachClick([]() { WS_running_inside = true; WS_running = false; });
    btnOK.attachLongPressStart([]() { WS_running_inside = false; WS_running = false; });
