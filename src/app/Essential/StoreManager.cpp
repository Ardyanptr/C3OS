...
// Binary-safe struct for caching
struct StoreApp_Cache {
    char name[32];
    char url[128];
    char file[32];
    char desc[128];
...
void onInstall_Store() { setView_Store(VIEW_INSTALLING_Store); }

void onOverviewBack_Store() { setView_Store(VIEW_LIST_Store); }

void onWiFiRetry_Store() { setView_Store(VIEW_LIST_Store); }
...
void drawOverview_Store() {
    ...
    display.setFont(u8g2_font_6x10_tr);
    display.drawStr(2, 10, app.name.c_str());
...
// ================= NETWORK =================
void fetchStore_Store(const char *url) {
    if (C3FS_Cache::refineCache("store", sizeof(StoreApp_Cache))) {
        int count = C3FS_Cache::getCacheCount("store", sizeof(StoreApp_Cache));
        storeItems_Store.clear();
        for (int i = 0; i < count; i++) {
            StoreApp_Cache c;
            if (C3FS_Cache::readCacheItem("store", i, &c, sizeof(StoreApp_Cache))) {
                storeItems_Store.push_back({String(c.name), String(c.url),
                                            String(c.file), String(c.desc),
                                            String(c.license), String(c.author)});
            }
        }
        needRedraw_Store = true;
    }
    ...
    if (http.begin(client, url) && http.GET() == HTTP_CODE_OK) {
        ...
        storeItems_Store.clear();
        std::vector<StoreApp_Cache> cacheItems;

        for (JsonObject a : doc["apps"].as<JsonArray>()) {
            StoreApp_Store app = {a["name"].as<String>(), a["url"].as<String>(),
                                  a["file"].as<String>(), a["desc"].as<String>(),
                                  a["license"].as<String>(), a["author"].as<String>()};
            storeItems_Store.push_back(app);

            StoreApp_Cache c;
            memset(&c, 0, sizeof(c));
            strncpy(c.name, app.name.c_str(), sizeof(c.name)-1);
            strncpy(c.url, app.url.c_str(), sizeof(c.url)-1);
            strncpy(c.file, app.file.c_str(), sizeof(c.file)-1);
            strncpy(c.desc, app.desc.c_str(), sizeof(c.desc)-1);
            strncpy(c.license, app.license.c_str(), sizeof(c.license)-1);
            strncpy(c.author, app.author.c_str(), sizeof(c.author)-1);
            cacheItems.push_back(c);
        }
        C3FS_Cache::saveCache("store", cacheItems.data(), sizeof(StoreApp_Cache), cacheItems.size());
    }
    http.end();
}
