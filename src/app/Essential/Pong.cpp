...
void runPongGame() {
    ...
    while (true) {
        ...
        if (game.gameRunning && now - game.lastUpdate > 20) {
            ...
            if (game.ballX <= 3 && game.ballX >= 0 && game.ballY >= game.paddleY && game.ballY <= game.paddleY + 15) {
                game.ballX = 3;
            ...
            if (game.ballX >= 125 && game.ballX <= 128 && game.ballY >= game.aiPaddleY && game.ballY <= game.aiPaddleY + 15) {
                game.ballX = 125;
                game.ballSpeedX = -fabs(game.ballSpeedX);
        ...
        for (int y = 0; y < 64; y += 4) display.drawPixel(64, y);
